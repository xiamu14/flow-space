class TreeNode {
  ...
  // 插入节点
  insertNode(pattern: string, parts: string[], height: number, element: JSX.Element) {
    ...
    if (!child) {
      child = new TreeNode(part, part[0] === ":" || part[0] === "*", "", [], element);
...
export class Router {
  ...
  // 获取路由
  getRoute(path: string): { route: TreeNode; params: Record<string, string> } | undefined {
    // 获取并解析路径
